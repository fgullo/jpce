 ...
 */
public class CEFeaturesProjectiveClusteringSim extends ProjectiveClusteringFeaturesSimilarity
{
    private ProjectiveClusterFeaturesSimilarity featuresSim;

    public CEFeaturesProjectiveClusteringSim (ProjectiveClusterFeaturesSimilarity featuresSim)
    {
        super(false);
        this.featuresSim = featuresSim;
        //this.onlyHard = false;
    }
    
    public CEFeaturesProjectiveClusteringSim (ProjectiveClusterFeaturesSimilarity featuresSim, boolean b)
    {
        super(b);
        this.featuresSim = featuresSim;
    }
    ...
    private double[][] computeSimConfusionMatrix(ProjectiveCluster[] partition1, ProjectiveCluster[] partition2)
    {
        ...
        for (int i=0; i<confusionSimMatrix.length; i++)
        {
            ...
            {
                confusionSimMatrix[i][j] = this.featuresSim.getSimilarity(partition1[i], partition2[j]);
            }
        } 
    ...
    private double[][] computeDistConfusionMatrix(ProjectiveCluster[] partition1, ProjectiveCluster[] partition2)
    {
        ...
        for (int i=0; i<confusionDistMatrix.length; i++)
        {
            ...
            {
                confusionDistMatrix[i][j] = this.featuresSim.getDistance(partition1[i], partition2[j]);
            }
        }
    ...
    {
        if (!(o instanceof CEFeaturesProjectiveClusteringSim))
        {
            return false;
        }
