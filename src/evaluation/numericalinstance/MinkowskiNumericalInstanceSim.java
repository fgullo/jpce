...
public class MinkowskiNumericalInstanceSim extends NumericalInstanceSimilarity {
     ...
     */
    public MinkowskiNumericalInstanceSim (int p) {
        this.p=p;
    }
    ...
    public double getDistance (Instance i1, Instance i2) 
    {
        ...
        for(int i=0; i<data.length;i++){
            sum = sum + Math.pow(Math.abs(data[i]-data1[i]), p); 
        }
    ...
    {
        if (!(o instanceof MinkowskiNumericalInstanceSim))
        {
            return false;
        }
