...
public class EMlikePCE_HYBRID extends PCEMethod implements EMlikeMethod
{
    ...
    public EMlikePCE_HYBRID (ProjectiveClusteringDataset ensemble)
    {
        ...
        while (currentIt <= this.maxIterations && !stop)
        {
            ...
            for (int c=0; c<clusterSolution[0].length; c++)
            {
                ...
                for (int j=0; j<clusterSolution.length; j++)
                {
                    ...
                    currentError += Ajc[j]*Math.pow(clusterSolution[j][c], this.m);
                }
                ...
                else
                {
                    ...
                    {
                        sum += Math.pow(((double)1.0)/Ajc[j],((double)1.0)/(this.m-1));
                    }
                    
                    double sumCheck = 0.0;
                    for (int j=0; j<clusterSolution.length; j++)
                    {
                        double value1 = Math.pow(Ajc[j], ((double)1.0)/(this.m-1))*sum;
                        ...
                        if (Math.abs(value-clusterSolution[j][c]) > 0.00000001)
                        {
                            stop = false;
                        }
                        ...
                        clusterSolution[j][c] = new Double(value);                        
                    }
            ...
            for (int l=0; l<objectSolution[0].length; l++)
            {
                ...
                for (int j=0; j<objectSolution.length; j++)
                {
                    ...
                    {
                        term += Math.pow(clusterSolution[j][c],this.m)*(1-this.allClusterObjectRep[c][l]);
                    }
                    zjl[j] = term;
            ...
            {              
                ...
                for (int i=0; i<featureSolution[j].length; i++)
                {
                    ...
                    {
                        termNew += Math.pow(clusterSolution[j][c],this.m)*(1-this.allClusterFeatureRep[c][i]);
                    }
                    Aji[i] = termNew;
    ...
    protected void randomInizialization(Double[][] objectSolution, Double[][] featureSolution, Double[][] clusterSolution, double[] kj, int nClusters)
    {
        ...
        for (int j=0; j<clusterSolution[0].length; j++)
        {
            ...
            for (int z=0; z<clusterSolution.length; z++)
            {
                ...
                sum += clusterSolution[z][j];
            }

            for (int z=0; z<clusterSolution.length; z++)
            {
                clusterSolution[z][j] /= sum;
            }
        }
    }
