...
public class EMlikePCE_ObjectByFeatureMatrix_OPTIMIZED extends PCEMethod implements EMlikeMethod
{
    ...
    public EMlikePCE_ObjectByFeatureMatrix_OPTIMIZED (ProjectiveClusteringDataset ensemble)
    {
            ...
            for (int l=0; l<objectSolution[0].length; l++)
            {
                ...
                for (int j=0; j<objectSolution.length; j++)
                {
                    ...
                    for (int c=0; c<this.allClusterObjectRep.length; c++)
                    {
                        ...
                        {
                            term3 += this.allClusterObjectRep[c][l]*featureSolution[j][i]*this.allClusterFeatureRep[c][i];
                        }
                    }
                    */ 
                    ...
                    for (int c=0; c<this.allClusterObjectRep.length; c++)
                    {
                        ...
                        for (int i=0; i<this.allClusterFeatureRep[c].length; i++)
                        {
                            dist += Math.exp(100*Math.pow(this.allClusterFeatureRep[c][i]-featureSolution[j][i],2));
                        }

                        term += this.allClusterObjectRep[c][l]*dist;
                    }
                    zjl[j] = term;
                    */
                    ...
                    for (int c=0; c<this.allClusterObjectRep.length; c++)
                    {
                        ...
                        for (int i=0; i<this.allClusterFeatureRep[0].length; i++)
                        {
                            dist += Math.pow(featureSolution[j][i]-this.allClusterObjectRep[c][l]*this.allClusterFeatureRep[c][i],2);
                        }
                        term += dist;
                    }
                    zjl[j] = term;
                    */
                    ...
                    for (int i=0; i<this.allClusterFeatureRep[0].length; i++)
                    {
                        ...
                        for (int c=0; c<this.allClusterObjectRep.length; c++)
                        {
                            dist += this.allClusterObjectRep[c][l]*this.allClusterFeatureRep[c][i];
                        }
                        dist /= this.ensemble.getDataLength();

                        term += Math.pow(featureSolution[j][i]-dist,2);
                    }
                    zjl[j] = term;
                    */
    ...
    {
        //this.Al = new double[this.ensemble.getNumberOfInstances()];
        ...
        /*
        for (int l=0; l<this.Al.length; l++)
        {
            for (int c=0; c<this.allClusterObjectRep.length; c++)
            {
                this.Al[l] += this.allClusterObjectRep[c][l];
            }
        }
        */ 
