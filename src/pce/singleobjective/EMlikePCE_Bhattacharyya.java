...
public class EMlikePCE_Bhattacharyya extends PCEMethod implements EMlikeMethod
{
    ...
    public EMlikePCE_Bhattacharyya (ProjectiveClusteringDataset ensemble)
    {
    ...
    public ProjectiveClustering execute (int nClusters)
    {
        ...
        while (currentIt <= this.maxIterations && !stop)
        {
            ...
            for (int l=0; l<objectSolution[0].length; l++)
            {
                ...
                for (int j=0; j<objectSolution.length; j++)
                {
                    ...
                    for (int c=0; c<this.allClusterObjectRep.length; c++)
                    {
                        ...
                        {
                            termTmp += Math.sqrt(featureSolution[j][i]*this.allClusterFeatureRep[c][i]);
                        }

                        if (termTmp-1>0.000000001)
                        {
                            throw new RuntimeException("ERROR: termTmp must be lower than or equal to 1---termTmp="+termTmp);
                        }
                        if (termTmp > 1)
                        {
                            termTmp = 1;
                        ...
                        term += this.allClusterObjectRep[c][l]*(1-termTmp);
                    }
            ...
            for (int j=0; j<featureSolution.length; j++)
            {
                ...
                {
                    xj += objectSolution[j][l];
                }


                //N.B. if xj==0, then cluster j doesn't not have any object; so, the feature should have equal weight
                if (xj == 0.0)
                {
                ...
                else
                {
                    ...
                    for (int i=0; i<featureSolution[j].length; i++)
                    {
                        ...
                        for (int l=0; l<objectSolution[j].length; l++)
                        {
                            zkd += Math.pow(objectSolution[j][l],this.m)*this.Cli[l][i];
                        }
                        sum += zkd*zkd;
                        newAssignment[i] = zkd*zkd;
                    }

                    double sumCheck = 0.0;
                    for (int i=0; i<featureSolution[j].length; i++)
                    {
                        double value = newAssignment[i]/sum;
        ...
        System.out.println("EMlike Subspace Clustering Ensembles (BTCR): OnlineExecutionTime="+this.getOnlineExecutionTime());
    ...
    protected void initialization()
    {
        ...
        for (int l=0; l<this.Cli.length; l++)
        {
            for (int i=0; i<this.Cli[l].length; i++)
            {
                for (int c=0; c<this.allClusterObjectRep.length; c++)
                {
                    this.Cli[l][i] += this.allClusterObjectRep[c][l]*Math.sqrt(this.allClusterFeatureRep[c][i]);
                }
            }
        }
    }
