...
public class FeatureBasedFeaturePartitioningGAFunction extends PCEGAFunction
{
    protected double[][] coNonOccurrenceFeatureMatrix;
    
    public FeatureBasedFeaturePartitioningGAFunction(double[][] coNonOccurrenceFeatureMatrix)
    {
        this.coNonOccurrenceFeatureMatrix = coNonOccurrenceFeatureMatrix;
    }
            ...
            for (int j1=0; j1<featuresRep.length; j1++)
            {
                for (int j2=0; j2<featuresRep.length; j2++)
                {
                    if (j1 != j2)
                    {
                        ...
                        h += coeff;
                        valueTmp += coeff*this.coNonOccurrenceFeatureMatrix[j1][j2];                       
                    }
                }
            }
            value += (valueTmp/h);         
